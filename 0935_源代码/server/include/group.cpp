...
int Group::groupeInsert(int g_admin_id, const char *g_name, const char *g_intro)
{
    char sql_insert[2048];
    sprintf(sql_insert, "insert into group_ (g_admin_id,g_name,g_intro,g_num) values (\'%d\', \'%s\',\'%s\',\'%d\')",
            g_admin_id, g_name, g_intro, 1);
    if (connectGroupDatabase())
    {
