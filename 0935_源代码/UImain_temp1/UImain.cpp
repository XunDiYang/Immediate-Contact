...
//
    ...
    g_signal_connect(gaddbutton,"pressed",G_CALLBACK( cgadd_pressed),NULL);
...
void createInputWindow()
{
    ...
    **********************************************/
    ...
    chatFileChooser = gtk_file_chooser_button_new("选择发送的文件",GTK_FILE_CHOOSER_ACTION_OPEN);
    ...
    gtk_box_pack_start(GTK_BOX(hbox), chatFileChooser, TRUE, TRUE, 0);

    GtkWidget* infoHistory = gtk_button_new_with_label("消息记录");
    gtk_box_pack_start(GTK_BOX(hbox), infoHistory, TRUE, TRUE, 0);
    ...
    sendFileBtn = gtk_button_new_with_label("Send Message"); //发送消息按钮
    gtk_widget_set_size_request(sendFileBtn,60,40);
    g_signal_connect(G_OBJECT(sendFileBtn),"clicked",G_CALLBACK(buttonPressSendFile),NULL);
    gtk_fixed_put(GTK_FIXED(mainFixed),sendFileBtn,750,740);
}
...
void createChatWindow(int ID)
{
    ...
    chatName = gtk_label_new(IDstring.c_str());
    gtk_fixed_put(GTK_FIXED(mainFixed), chatName, 380, 30);
...
void createFriendsList()
{
    ...
    **********************************************/
    ...
    for( int i=0 ;i<current_friend_num; i++ )
    {
        ...
        anniuLabel = gtk_label_new("FRIENDLIST");
...
void createGroupList()
{
    ...
    for( int i=0 ;i<GROUPS_NUM; i++ )
    {
        ...
        anniuLabel = gtk_label_new("GROUPNAME");
...
{
    char *chatFileUri;
    chatFileUri = gtk_file_chooser_get_uri(GTK_FILE_CHOOSER(chatFileChooser));
    client_send_file(current_id,talk_to_user,chatFileUri);
}
void buttonPressSendMessage()
{
    ...
    messagePrint("\n\n");
    talk_to_user = 22;
    send_message_callback(chatInfo);
...
/************************************************************************
*名称:judgeAcceptedFile,buttonPressAccept,buttonPressRefuse
*描述:弹出是否接收文件窗口
*作成日期:2019.09.05
*参数:无
*返回值:int
